## Version $VER: jabber.module.catalog 2.0 (25.03.2022)
## Languages english polski
## Codeset english 0
## Codeset polski 0
## SimpleCatConfig CharsPerLine 200
## TARGET C english "translations.h" NoCode
## TARGET CATALOG polski "bin/catalogs/polski" Optimize
MSG_PREFS_BASIC
User Settings
Ustawienia użytkownika
;
MSG_PREFS_BASIC_ID
Jabber ID:
Jabber ID:
;
MSG_PREFS_BASIC_PASS
Password:
Hasło:
;
MSG_PREFS_BASIC_ID_HELP
Your Jabber ID, format: user@server.com.
Twój Jabber ID w formacie użytkownik@serwer.com.
;
MSG_PREFS_BASIC_PASS_HELP
Your password.
Twoje hasło.
;
MSG_PREFS_CONNECTION
Connection Settings
Ustawienia połączenia
;
MSG_PREFS_CONNECTION_ADVANCED_HELP
Allows to set server addres and port number manually.
Zaznacz, aby móc ręcznie zdefiniować szczegóły połączenia.
;
MSG_PREFS_CONNECTION_ADVANCED
Turn on advanced connection settings
Włącz zaawansowane ustawienia połączenia
;
MSG_PREFS_CONNECTION_SERVER_HELP
Address of server you want connect to.
Adres serwera, z którym chcesz się połączyć.
;
MSG_PREFS_CONNECTION_SERVER
Server:
Serwer:
;
MSG_PREFS_CONNECTION_PORT
Port:
Port:
;
MSG_PREFS_CONNECTION_PORT_HELP
Server's port you want connect to.
Port serwera, na którym chcesz się połączyć.
;
MSG_MENU_DEBUGLOG_OPEN
Debug Log...
Konsola debugowania...
;
MSG_DEBUGLOG_WINDOW_TITLE
KwaKwa: Jabber Debug Log
KwaKwa: Konsola debugowania Jabbera
;
MSG_DEBUGLOG_WINDOW_LEGEND_OUTGOING
Outgoing Data
Dane wychodzące
;
MSG_DEBUGLOG_WINDOW_LEGEND_INCOMING
Incoming Data
Dane przychodzące
;
